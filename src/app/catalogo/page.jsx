...
export default function Products() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        setFeaturedProducts(data.filter(p => p.featured && p.stock > 0).slice(0, 6));
        setOfferProducts(data.filter(p => p.categories?.includes('Ofertas') && p.stock > 0).slice(0, 3));
  ...
  const handleCategorySelect = category => setFilters({ ...filters, category, subcategories: [] });
    ...
    <div className="min-h-screen text-[#6b554b] bg-gray-50 pt-20">
      <main className="max-w-6xl mx-auto px-4 py-2">
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-600">No se encontraron productos con los filtros aplicados.</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
