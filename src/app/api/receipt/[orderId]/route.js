...
export async function GET(_req, { params }) {
  try {
    // 1) Cargar pdfmake en runtime (no en build)
    const { default: PdfPrinter } = await import("pdfmake");

    // 2) Declarar fuentes estándar PDF (sin TTF)
    const fonts = {
      Times: {
        normal: "Times-Roman",
        bold: "Times-Bold",
    ...
    const docDefinition = {
      ...
      content: [
        ...
        { text: "Este documento es un comprobante emitido por el comercio en base a la transacción procesada por Webpay.", style: "foot", margin: [0, 12, 0, 2] },
