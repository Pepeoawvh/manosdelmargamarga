...
export default function HeroCarousel() {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const slidesData = snapshot.docs
          ...
          .filter((slide) => slide.visible); // Solo slides visibles
  ...
  if (error)
    return (
      <div className="h-[500px] bg-gray-100 flex items-center justify-center text-red-500">
        Error: {error}
      </div>
    );
  if (slides.length === 0)
    return (
      <div className="h-[500px] bg-gray-100 flex items-center justify-center text-gray-500">
